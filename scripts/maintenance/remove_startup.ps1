...
try{
 ...
 if($removed.Count){Write-Host '已移除下列本專案自動啟動設定：' -ForegroundColor Green;$removed|ForEach-Object{Write-Host (" - {0}" -f $_)}}else{Write-Host '未發現本專案的自動啟動設定。' -ForegroundColor Green}
 Finish 0
